...
func (a *Agent) processSessionOpen(pkt *pb.Packet) {
	...
	if a.config.AgentMode == pb.AgentModeEmbeddedType {
		for _, envKeyVal := range os.Environ() {
			...
			if !found || envKey == "BIFROST_DSN" || envKey == "BIFROST_KEY" {
				continue
			}
	...
	// Store connection params BEFORE sending SessionOpenOK to ensure they're available
	// when subsequent packets (like MySQLConnectionWrite) arrive
...
func (a *Agent) executePostgresCommand(psqlCmd string) ([]byte, int) {
